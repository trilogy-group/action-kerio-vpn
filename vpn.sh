...
# Variables passed as arguments to the script
...
# Ensure apt package list is updated and required packages are installed
sudo apt update
sudo apt install -y wget curl debconf openssl

# Function to set the MTU for the VPN interface
setMTU() {
...
# Function to wait for the kvnet interface to become ready
waitForKvnet() {
    start_time=$(date +%s)
    while ! ip a show kvnet up | grep inet >/dev/null 2>&1; do
        current_time=$(date +%s)
...
# Write VPN configuration parameters for the installation
...
setupVpn() {
  ...
  if [ ! -f "/tmp/kerio-control-vpnclient-${VPN_CLIENT_VERSION}-linux-amd64.deb" ]; then
    wget -O /tmp/kerio-control-vpnclient-${VPN_CLIENT_VERSION}-linux-amd64.deb "http://cdn.kerio.com/dwn/control/control-${VPN_CLIENT_VERSION}/kerio-control-vpnclient-${VPN_CLIENT_VERSION}-linux-amd64.deb"
  fi
...
# Attempt to verify the VPN connection
retry_count=1
try_verify
while [ $curl_retval -ne 0 ] && [ $retry_count -lt $MAX_CURL_ATTEMPT ]; do
  echo "curl failed with ${curl_retval}. retrying (${retry_count})..."
  retry_count=$((retry_count + 1))
  sleep 3  # Added delay between retries for better stability
  try_verify
done
